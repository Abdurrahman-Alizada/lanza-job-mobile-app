...
export default function FuturShifts() {
        ...
        <View style={{flexDirection:'row', padding:'2%', flexWrap:'wrap'}}>
            {
                filterChips.map((item,index)=>(
                    <Chip
                   // selected={item==selectedFilterChips ? true : false}
                    //mode={item==selectedFilterChips ? 'outlined' : 'flat'}

                    style={{marginTop:'4%', marginRight:'4%', borderRadius:15,
                      backgroundColor: item === selectedFilterChips ? 'blue' : '#c0c0c0',
                      
                    }}
                    textStyle={{color: item === selectedFilterChips ? 'white' : 'black'}}
                    key={index}
                    onPress={()=>setSelectedFilterChips(item)}>{item}</Chip>
                ))
            }
      ...
      >
        ...
        <View style={{ flexDirection: 'row', marginLeft: 15, marginTop: 10, justifyContent: "space-between", alignItems: "center" }}>
          ...
          <Button style={{ borderRadius: 8,
                           ...
                           }} onPress={() => navigation.navigate("JobDetailsScreen", {jobId:item._id})}>
             <Text style={{color:theme.colors.background}}>You Cancelled
            </Text>
          </Button>
        </View>
      </Card>
